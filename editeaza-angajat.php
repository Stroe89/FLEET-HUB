...
// Preluăm ID-ul angajatului din URL
$id_angajat = $_GET['id'] ?? null;
$angajat = null; // Variabila pentru a stoca datele angajatului

if ($id_angajat && is_numeric($id_angajat)) {
    // Preluăm datele angajatului din baza de date
    $stmt = $conn->prepare("SELECT * FROM angajati WHERE id = ?");
    if ($stmt === false) {
        die("Eroare la pregătirea interogării: " . $conn->error);
    }
    $stmt->bind_param("i", $id_angajat);
    $stmt->execute();
    ...
    if ($result->num_rows > 0) {
        ...
        // Redirecționează sau afișează o eroare dacă angajatul nu există
        // header("Location: lista-angajati.php");
        // exit();
    }
    $stmt->close();
} else {
    $error_message = "ID angajat invalid sau lipsă.";
    // header("Location: lista-angajati.php");
    // exit();
}

// Dacă angajatul nu a fost găsit, inițializăm cu valori goale pentru a evita erori PHP
if (!$angajat) {
    $angajat = [
        'nume' => '',
        'prenume' => '',
...
<title>NTS TOUR | Editează Angajat</title>
...
<main class="main-wrapper">
    <div class="main-content">
        ...
        <div class="page-breadcrumb d-none d-sm-flex align-items-center mb-3">
            <div class="breadcrumb-title pe-3">Angajați</div>
            <div class="ps-3">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb mb-0 p-0">
                        <li class="breadcrumb-item"><a href="index.php"><i class="bx bx-home-alt"></i></a></li>
                        <li class="breadcrumb-item"><a href="lista-angajati.php">Listă Angajați</a></li>
                        <li class="breadcrumb-item active" aria-current="page">Editează Angajat</li>
                    </ol>
                </nav>
            </div>
        </div>
        ...
        <div class="row">
            <div class="col-12">
                <div class="card">
                    <div class="card-body">
                        <h4 class="card-title">Editează Angajat: <?php echo htmlspecialchars($angajat['nume'] . ' ' . $angajat['prenume']); ?></h4>
                        <hr>
                        ...
                        <?php if (!$angajat['nume']): // Verificăm dacă angajatul a fost găsit ?>
                            <div class="alert alert-warning" role="alert">
                                Angajatul nu a fost găsit sau ID-ul este invalid. Te rugăm să te întorci la <a href="lista-angajati.php" class="alert-link">lista de angajați</a>.
                            </div>
                        <?php else: ?>
                            <form id="editAngajatForm" action="process_angajati.php" method="POST" class="needs-validation" novalidate>
                                <input type="hidden" name="action" value="edit">
                                <input type="hidden" name="id" value="<?php echo htmlspecialchars($id_angajat); ?>">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const editAngajatForm = document.getElementById('editAngajatForm'); // Am schimbat ID-ul formularului

    // Funcție pentru generarea unui cod intern unic (păstrată de la adauga-angajat.php)
    ...
    // Generează codul la click pe buton (pentru editare, codul intern nu se generează automat la load)
    if (generateCodeBtn) { // Verificăm dacă butonul există (doar dacă angajatul a fost găsit)
    ...
    if (editAngajatForm) { // Asigură-te că formularul există înainte de a adăuga event listener
        editAngajatForm.addEventListener('submit', function(event) {
            event.preventDefault(); // Oprește trimiterea normală a formularului

            // Validare Bootstrap client-side
            if (!editAngajatForm.checkValidity()) {
                event.stopPropagation();
                editAngajatForm.classList.add('was-validated');
                showToast('danger', 'Te rog completează toate câmpurile obligatorii.');
                return;
            }
            editAngajatForm.classList.add('was-validated'); // Asigură că validarea vizuală e aplicată

            const formData = new FormData(editAngajatForm);
            ...
            .then(data => {
                if (data.success) {
                    ...
                    // Opțional: redirecționează către fișa individuală a angajatului după un scurt delay
                    setTimeout(() => {
                        window.location.href = `fise-individuale.php?id=${formData.get('id')}`;
